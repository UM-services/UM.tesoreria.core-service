...
public class ContableService {
	...
	public void recalculateGrados() {
		...
		for (Cuenta cuenta : cuentas) {
			...
			cuenta.setGrado(5);
			cuenta.setGrado1(cuenta.getNumeroCuenta().divideToIntegralValue(new BigDecimal(10000000000.0))
					.multiply(new BigDecimal(10000000000.0)));
			cuenta.setGrado2(cuenta.getNumeroCuenta().divideToIntegralValue(new BigDecimal(100000000.0))
					.multiply(new BigDecimal(100000000.0)));
			cuenta.setGrado3(cuenta.getNumeroCuenta().divideToIntegralValue(new BigDecimal(1000000.0))
					.multiply(new BigDecimal(1000000.0)));
			cuenta.setGrado4(cuenta.getNumeroCuenta().divideToIntegralValue(new BigDecimal(10000.0))
					.multiply(new BigDecimal(10000.0)));
	...
	public static Boolean isSaldoDeudor(BigDecimal cuenta) {
		...
		if (digitos.contains(cuenta.divide(new BigDecimal(10000000000.0)).intValue()))
			return true;
		return false;
	}
	...
	public List<BigDecimal> saldoInicial(BigDecimal cuenta, Ejercicio ejercicio, OffsetDateTime desde) {
		...
		if (desde.compareTo(ejercicioInicio) == 0) {
	...
	public AsientoInternal nextAsiento(OffsetDateTime fechaContable, List<AsientoInternal> asientos) {
		...
		if (asientos != null) {
			for (AsientoInternal asiento : asientos) {
				if (asiento.getFechaContable().compareTo(fechaContable) == 0) {
	...
	@Transactional
	public Boolean deleteAsiento(OffsetDateTime fechaContable, Integer ordenContable)
			throws EjercicioBloqueadoException {
	...
	public Boolean deleteAsientoDesde(OffsetDateTime fechaContable, Integer ordenContable, Integer item) {
		...
		try {
			...
			for (CuentaMovimiento cuentaMovimiento : cuentaMovimientoService.findAllByAsiento(fechaContable,
					ordenContable, 0, 2)) {
