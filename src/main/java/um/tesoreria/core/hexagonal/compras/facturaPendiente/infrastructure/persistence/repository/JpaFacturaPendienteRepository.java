package um.tesoreria.core.hexagonal.compras.facturaPendiente.infrastructure.persistence.repository;

import jakarta.transaction.Transactional;
import org.springframework.data.jpa.repository.Modifying;
import org.springframework.data.jpa.repository.Query;
import um.tesoreria.core.hexagonal.compras.facturaPendiente.infrastructure.persistence.entity.FacturaPendienteEntity;
...
public interface JpaFacturaPendienteRepository extends JpaRepository<FacturaPendienteEntity, Long> {
    ...
    @Query(
        value = """
SELECT DISTINCT m.mvp_id AS proveedor_movimiento_id, p.`Prv_Razon` AS razon_social, p.`Prv_CUIT` AS cuit, p.cbu, m.`MvP_FechaComprob` AS fecha_comprobante
, m.`MvP_FechaVenc` AS fecha_vencimiento, m.`MvP_Concepto` AS observaciones
...
ORDER BY p.prv_id, m.`MvP_FechaComprob`    """, nativeQuery = true)
    List<FacturaPendienteEntity> findFacturasPendientes(
