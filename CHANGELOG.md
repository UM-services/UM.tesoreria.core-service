...
## [4.6.0] - 2026-09-24
### Added
- feat(chequera/chequeraSerie): Nuevo endpoint paginado `GET /chequeraserie/usuario/{userId}/lectivo/{lectivoId}` (alias `/api/tesoreria/core/chequeraSerie/...`) con las chequeras de las facultades asignadas al usuario. `ChequerasPorUsuarioService` resuelve las facultades vía `UsuarioChequeraFacultadService`, admite el filtro opcional de alumno (`personaId` + `documentoId`, ambos o ninguno; 400 si el filtro o cualquier parámetro es inválido), pagina (por defecto 20, máximo 100, orden `chequeraId DESC`) y calcula la deuda vencida de cada chequera con `CalculateDeudaUseCase`. El contrato `ChequeraEstadoUsuarioPageResponse` (`content`, `totalElements`, `number`, `size`, `totalPages`) contiene `ChequeraEstadoUsuarioResponse` con `titular` ("Apellido, Nombre"), nombres de facultad/tipo de chequera, `becaPorcentaje`, `importeDeuda`, `cuotasDeuda`, `estadoDeuda` (`CON_DEUDA_VENCIDA`/`SIN_DEUDA_VENCIDA`) y flags `asentado`/`enviado`/`retenida`.
- feat(chequera/chequeraSerie): Nuevas firmas paginadas en el puerto `ChequeraSerieRepository` (`findAllByLectivoIdAndFacultadIdIn` y `findAllByLectivoIdAndFacultadIdInAndPersonaIdAndDocumentoId`, ambas `Page<ChequeraSerie>`) implementadas por `JpaChequeraSerieRepository`/`JpaChequeraSerieRepositoryAdapter`, y nuevo método `ChequeraSerieMapper.toEstadoUsuarioDomain` que carga solo los nombres de `facultad`/`tipoChequera`/`persona` y normaliza `becaPorcentaje` nulo a `0`.
- feat(chequera/chequeraSerie): `becaPorcentaje` se expone en el estado JSON como fracción (`0.5` = 50%, documentado con `@Schema`) y en el PDF de estado como "Porcentaje de beca" (`movePointRight(2).stripTrailingZeros()` con `null` tratado como `0`).
- feat(chequera): Nuevo endpoint `GET /chequera/generateEstadoPdf/{facultadId}/{tipoChequeraId}/{chequeraSerieId}/{alternativaId}/{debitoTipoId}` que genera y descarga el PDF "Estado de Chequera" (`estado-chequera.pdf`) vía `FormulariosToPdfService.generateEstadoChequeraPdf`: encabezado con logo y título (sin línea de acento), datos del titular (titular, tipo de chequera, arancel, lectivo, porcentaje de beca, tipo de impresión), bloques de producto (Matrícula primero y luego Arancel) con columna del primer vencimiento contractual (`vencimiento1` en `dd/MM/yyyy`), planilla de débitos con su vencimiento y leyenda "NO VALIDO COMO COMPROBANTE DE PAGO"; responde 500 si la generación falla.
- feat(legacy/tipoImpresion): Nuevos modelo `TipoImpresion` (tabla `tipoimpresion`, columnas `tim_id`/`tim_nombre`), `TipoImpresionRepository.findByTipoImpresionId`, `TipoImpresionService` y `TipoImpresionException` para resolver el tipo de impresión en el encabezado del estado PDF.
- feat(personas/persona): Nuevo endpoint `GET /persona/sugerencias/usuario/{userId}?q=&limite=8` (alias `/api/tesoreria/core/persona/...`) con sugerencias de personas que tienen chequeras en las facultades asignadas al usuario. `PersonaSugerenciaService` valida `userId > 0`, `q` de hasta 120 caracteres con al menos 3 alfanuméricos y `limite` 1..20 (400 si es inválido) y divide `q` en términos (espacio o coma) que deben coincidir todos (AND). `JdbcPersonaSugerenciaRepositoryAdapter` consulta `persona`/`documento` con `EXISTS` sobre `chequera_serie` unido a `usuario_chequera_facultad` por `facultad_id = chs_fac_id`, `LIKE ... ESCAPE '!'` con `%`/`_` escapados y ranking que prioriza apellidos que empiezan con el primer término y luego los que lo contienen como palabra. Nuevo puerto de salida `PersonaSugerenciaRepository`, registro de dominio `PersonaSugerencia` y DTO `PersonaSugerenciaResponse`.
- feat(auth): Nuevo puerto de entrada `ChangePasswordUseCase` e `ChangePasswordUseCaseImpl` con validaciones en orden: clave nueva obligatoria ("ERROR: Falta CLAVE . . ."), coincidencia con `reClaveNueva` si viene ("ERROR: Claves NO Coinciden"), resolución del usuario por `userId` y luego `login` ("ERROR: Usuario NO Encontrado"), bloqueo de cuentas cuyo login empieza con "admin" ("ERROR: NO se puede Cambiar ESTA Clave"), verificación de la clave anterior con SHA-256 ("ERROR: Usuario NO Autenticado") y rechazo de claves ya usadas por otro usuario ("ERROR: Clave NO Válida"); al éxito actualiza el hash y el `nombre` opcional y persiste vía `UsuarioAuthRepository.save`. Expuesto en `POST /api/tesoreria/core/auth/change-password` con `ChangePasswordRequest` (`currentPassword`/`newPassword` con `@NotBlank`), errores de validación → 400.
- feat(auth): Nuevo endpoint `GET /api/tesoreria/core/auth/me/{userId}` que devuelve el `LoginResponse` del usuario (404 si no existe), soportado por `AuthService.findById` y el nuevo método del puerto `UsuarioAuthRepository.findById` implementado en `JpaUsuarioAuthRepositoryAdapter`.
- test: Nuevas pruebas `ChangePasswordUseCaseImplTest`, `AuthControllerTest`, `ChequerasPorUsuarioServiceTest`, `ChequeraSerieUsuarioControllerTest`, `JpaChequeraSerieUsuarioQueryTest`, `FormulariosToPdfServiceGenerateEstadoChequeraPdfTest` y `TipoImpresionServiceTest`; `AuthDtoMapperTest` extiende la cobertura al campo `login`.
- test/infra: Nuevo perfil Maven `it` (failsafe, `mvn -Pit verify`) que ejecuta los `*IT` (`ChequerasPorUsuarioIT`, `PersonaSugerenciasIT`) contra MySQL de solo lectura vía `application-it.yml` (`ddl-auto: none`, Consul/Kafka/mail deshabilitados, variables `IT_DB_*`); el README documenta su uso con un archivo `.env` ignorado por Git.
- feat(docs): Diagramas `hexagonal-auth.mmd`, `hexagonal-chequeraSerie.mmd` y `hexagonal-persona.mmd` sincronizados con el código (v4.6.0) y registro de `docs/README.md` actualizado.

### Changed
- feat(auth): `LoginResponse` incorpora el campo aditivo `login`, mapeado por `AuthDtoMapper` desde `UsuarioAuth`; `LoginRequest` añade `@NoArgsConstructor`/`@AllArgsConstructor` (el contrato JSON de `/auth` no cambia; los constructores habilitan `new LoginRequest(login, password)` en pruebas).
- fix(chequera): El encabezado del estado PDF elimina la línea de acento entre el logo/título y los datos del titular.
- chore(test): `src/test/resources/application.yml` reemplaza la contraseña SMTP hardcodeada por el placeholder `unused-test-password`.
- chore: `.gitignore` añade `/.conductor/`, `/.context/` y `/.env`.

> Basado en `git diff b3528dc..HEAD` (32 archivos, +1761/−4 líneas: PRs #378, #379 y #380 posteriores a v4.5.0), `git diff HEAD` (staged: 13 archivos, +404 líneas, slice `auth`), el código Java real de `ChequerasPorUsuarioService`, `PersonaSugerenciaService`/`JdbcPersonaSugerenciaRepositoryAdapter`, `FormulariosToPdfService.generateEstadoChequeraPdf`, `TipoImpresion`, `ChangePasswordUseCaseImpl`, los DTOs y pruebas, y `pom.xml` (versión `4.5.0` → `4.6.0`). Todos los cambios son endpoints, puertos, contratos JSON aditivos y capacidades nuevas sin ruptura ni eliminación de APIs públicas existentes (el `login` de `LoginResponse` es aditivo y `estadoDeuda`/`becaPorcentaje` aparecen solo en el nuevo DTO); corresponde un incremento minor de SemVer.
...
- chore(politicaArancelaria): Añadidos 3 statements de debug logging en `RecalculateCuotaByUniqueIndexUseCaseImpl`
  - `log.debug` para cuota en revisión, cuota referencia e importe referencia
